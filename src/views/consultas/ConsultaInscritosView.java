...
import controllers.ConsultaInscricaoController;
...
public class ConsultaInscritosView extends JFrame {
	...
	ConsultaInscricaoController controller = new ConsultaInscricaoController();
	List<Inscricao> list;
	...
	private void initializeComponent() {
		...
		btnSearch.setIcon(new ImageIcon(ConsultaInscritosView.class.getResource("/resources/icons/search.png")));
	...
	private void loadData() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			try {
				...
				Object[] row = { item.getCodigo(), disciplina.getNome(), professor.getCpf(), professor.getNome(), professor.getAreaConhecimento(), professor.getPontuacao(), item.getStatus() };
